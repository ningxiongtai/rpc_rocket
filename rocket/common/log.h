...
private:
  ...
  std::string m_file_name;//日志文件输出名字
  std::string m_file_path;//日志文件输出路径
  int m_max_file_size {0};//单位为字节
  
  sem_t m_sempahore;
  pthread_t m_thread;
 
  pthread_cond_t m_condtion;  // 条件变量 拼写错误
  Mutex m_mutex;
  
  std::string m_date;//当前打印日志的文件日期
  FILE* m_file_hanlder {NULL};//当前打开日志文件句柄
