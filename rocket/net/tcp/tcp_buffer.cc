...
namespace rocket {
    ...
    void TcpBuffer::moveReadIndex(int size) {
        ...
        if(j >= m_buffer.size()) {
            ERRORLOG("moveReadIndex error, invaild size %d, old_read_index %d , buffer size %d", size, m_read_index, m_buffer.size());
            return ;
        }
    ...
    void TcpBuffer::moveWriteIndex(int size) {
        ...
        if(j >= m_buffer.size()) {
            ERRORLOG("moveWriteIndex error, invaild size %d, old_read_index %d , buffer size %d", size, m_write_index, m_buffer.size());
            return ;
        }
